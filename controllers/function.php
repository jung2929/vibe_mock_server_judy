...
function isValidHeader($jwt, $key)
{
    try {
        ...
        //로그인 함수 직접 구현 요함
         return isValidUser($data->customerEmail, $data->pw);
    } catch (\Exception $e) {
        return false;
    }
}
...
function getJWToken($customerEmail, $pw, $secretKey)
{
    $data = array(
...
function getUserToken($customerEmail, $pw,$contactInformation, $customerId,$secretKey)
{
    $data = array(
        'date' => (string)getTodayByTimeStamp(),
        'customerEmail' => (string)$customerEmail,
